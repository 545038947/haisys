...
class paipai{
	...
	public function __construct($appuser,$paipai_set){
		...
		$this->nowords="3";//dd_get_cache('no_words'.$noword_tag);
	}
	...
	function get($api_name,$parame){
		...
		if(!isset($b['errorCode']) || $b['errorCode']==0){
			...
			if($this->errorlog==1){
				...
				create_file(APPROOT.'/Runtime/Temp/paipai_error_log/'.date('Ymd').'.txt',$error,1,1);
			}
			return 102;
		}
	}
	...
	public function cpsCommSearch($parame){
		...
		foreach($row['vecComm'] as $k=>$a){
			...
			//$arr['fxje']=fenduan($arr['crValue'],$this->fxbl,$this->appuser['level']);
	...
	function cpsCommQueryAction($parame){
		...
		if($row['dwActiveFlag']==1){
			...
			$arr['fxje']=fenduan($arr['commission'],$this->fxbl,$this->appuser['level']);
		}
